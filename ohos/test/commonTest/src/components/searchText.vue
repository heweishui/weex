  ...
  methods: {
    ...
    bindScroll(anchors, refs) {
      ...
      refs && Object.keys(refs).forEach((key) => {
        const reg = /After$/;
        const expression = reg.test(key) ? `0+y/${this.distance}` : `y/${this.distance}`;
        const item = {
          ...
          props: [
            {
              ...
              expression: `rgba(${this.backgroundColor[0]}, ${this.backgroundColor[1]}, ${this.backgroundColor[2]}, y/${distance})`,
            },
            {
              element: leftIcon,
              property: 'opacity',

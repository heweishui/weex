<template>
  <scroller class="scroller">
    <div class="wrapper">
      <text class="nav">multiPage1页面</text>
      <div class="btn-group">
        <text class="font-title">navigator</text>
        <text class="btn" @click="jumpPage('multiPage2')">push(multiPage2)</text>
...
export default {
  ...
  methods: {
    ...
    broadcastPost() {
      testbc.postMessage('来自multiPage1的消息test message!');
    },
    broadcastOnmessage() {
      testbc.onmessage = (event)=>{
        console.log("multiPage1收到消息：" + JSON.stringify(event.data));
      }
    },
    broadcastClose() {
      testbc.close();

<template>
  <scroller class="scroller">
    <div class="wrapper">
      <text class="nav">multiPage8页面</text>
      <text class="btn" @click="jumpPage('multiPage9')">push(multiPage9)</text>
...
export default {
  ...
  methods: {
    ...
    broadcastPost() {
      testbc.postMessage('来自multiPage8的消息test message!');
    },
    broadcastOnmessage() {
      testbc.onmessage = (event)=>{
        console.log("multiPage8收到消息：" + JSON.stringify(event.data));
      }
    },
    broadcastClose() {
      testbc.close();

<template>
  <scroller class="scroller">
    <div class="wrapper">
      <text class="nav">multiPage9页面</text>
      <text class="btn" @click="jumpPage('multiPage10')">push(multiPage10)</text>
...
export default {
  ...
  methods: {
    ...
    broadcastPost() {
      testbc.postMessage('来自multiPage9的消息test message!');
    },
    broadcastOnmessage() {
      testbc.onmessage = (event)=>{
        console.log("multiPage9收到消息：" + JSON.stringify(event.data));
      }
    },
    broadcastClose() {
      testbc.close();

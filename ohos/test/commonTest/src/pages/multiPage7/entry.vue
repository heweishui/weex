<template>
  <scroller class="scroller">
    <div class="wrapper">
      <text class="nav">multiPage7页面</text>
      <text class="btn" @click="jumpPage('multiPage8')">push(multiPage8)</text>
...
export default {
  ...
  methods: {
    ...
    broadcastPost() {
      testbc.postMessage('来自multiPage7的消息test message!');
    },
    broadcastOnmessage() {
      testbc.onmessage = (event)=>{
        console.log("multiPage7收到消息：" + JSON.stringify(event.data));
      }
    },
    broadcastClose() {
      testbc.close();

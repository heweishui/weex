<template>
  <scroller class="scroller">
    <div class="wrapper">
      <text class="nav">multiPage11页面</text>
      <text class="btn" @click="jumpPage('multiPage12')">push(multiPage12)</text>
...
export default {
  ...
  methods: {
    ...
    broadcastPost() {
      testbc.postMessage('来自multiPage11的消息test message!');
    },
    broadcastOnmessage() {
      testbc.onmessage = (event)=>{
        console.log("multiPage11收到消息：" + JSON.stringify(event.data));
      }
    },
    broadcastClose() {
      testbc.close();

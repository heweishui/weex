<template>
  <scroller class="scroller">
    <div class="wrapper">
      <text class="nav">multiPage6页面</text>
      <text class="btn" @click="jumpPage('multiPage7')">push(multiPage7)</text>
...
export default {
  ...
  methods: {
    ...
    broadcastPost() {
      testbc.postMessage('来自multiPage6的消息test message!');
    },
    broadcastOnmessage() {
      testbc.onmessage = (event)=>{
        console.log("multiPage6收到消息：" + JSON.stringify(event.data));
      }
    },
    broadcastClose() {
      testbc.close();

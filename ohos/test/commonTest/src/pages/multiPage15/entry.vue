<template>
  <scroller class="scroller">
    <div class="wrapper">
      <text class="nav">multiPage15页面</text>
      <text class="font-title">最后一个页面，逐级返回</text>
      ...
      <text class="btn" @click="navigatorPop(3)">pop(3)</text>
    </div>
  </scroller>
</template>
...
export default {
  ...
  methods: {
    ...
    broadcastPost() {
      testbc.postMessage('来自multiPage15的消息test message!');
    },
    broadcastOnmessage() {
      testbc.onmessage = (event)=>{
        console.log("multiPage15收到消息：" + JSON.stringify(event.data));
      }
    },
    broadcastClose() {
      testbc.close();

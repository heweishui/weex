<template>
  <scroller class="scroller">
    <div class="wrapper">
      <text class="nav">multiPage12页面</text>
      <text class="btn" @click="jumpPage('multiPage13')">push(multiPage13)</text>
...
export default {
  ...
  methods: {
    ...
    broadcastPost() {
      testbc.postMessage('来自multiPage12的消息test message!');
    },
    broadcastOnmessage() {
      testbc.onmessage = (event)=>{
        console.log("multiPage12收到消息：" + JSON.stringify(event.data));
      }
    },
    broadcastClose() {
      testbc.close();

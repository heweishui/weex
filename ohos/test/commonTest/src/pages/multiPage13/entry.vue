<template>
  <scroller class="scroller">
    <div class="wrapper">
      <text class="nav">multiPage13页面</text>
      <text class="btn" @click="jumpPage('multiPage14')">push(multiPage14)</text>
...
export default {
  ...
  methods: {
    ...
    broadcastPost() {
      testbc.postMessage('来自multiPage13的消息test message!');
    },
    broadcastOnmessage() {
      testbc.onmessage = (event)=>{
        console.log("multiPage13收到消息：" + JSON.stringify(event.data));
      }
    },
    broadcastClose() {
      testbc.close();

<template>
  <scroller class="scroller">
    <div class="wrapper">
      <text class="nav">multiPage10页面</text>
      <text class="btn" @click="jumpPage('multiPage11')">push(multiPage11)</text>
...
export default {
  ...
  methods: {
    ...
    broadcastPost() {
      testbc.postMessage('来自multiPage10的消息test message!');
    },
    broadcastOnmessage() {
      testbc.onmessage = (event)=>{
        console.log("multiPage10收到消息：" + JSON.stringify(event.data));
      }
    },
    broadcastClose() {
      testbc.close();
